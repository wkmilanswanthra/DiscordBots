...
class Info(Cog):
    ...
    async def user_info(self, ctx, *, name: Optional[str]):
        ...
        print("here")
        embed = Embed(title=f"Information about {target.nick or target.name}",
    ...
    @command(name="serverinfo", aliases=["si"], description="Display info about the server")
    async def server_info(self, ctx):
        embed = Embed(title=f"Information about {ctx.guild.name}",
        ...
        fields = [("ID", ctx.guild.id, False),
                  ...
                  ("Owner", ctx.guild.owner.name, True),
